...
# This version plots a line for each event
# """
...
# import appmgr.datacacher as datacacher
# import appmgr.datachecker as datachecker
# import appmgr.mouseover_maker as mouseover_maker
# import charts.commonchartfuncs as common
# import charts.palette as palette
# from charts.types import CustomizationOptions
# from datetime import datetime
...
#     customize_dict: Optional[CustomizationOptions] = None,
#     unassigned_colors: Optional[list] = None,
#     background_color: str = palette.DEFAULT_BG_COLOR,
#     axis_line_color: str = palette.DEFAULT_AXIS_LINE_COLOR,
#     events = {},
#     diff: bool = False,
#     absolute: bool = False,
#     bar: bool = False,
...
#     Also creates a diff graph if diff=True. 
#         - ToDo: (might be weird implementation) If given more than 2 y_vars, then it plots the difference of each of the y_vars with respect
#           to the first y_var
#     """
#     logging.debug("starting to make a new stacked plot graph")
#     graph = go.Figure()
#     graph.update_layout(
#         plot_bgcolor=background_color,
#         xaxis=dict(
...
#     if len(y_vars) == 0:
#         raise Exception("need at least one y variable")
        
#     if diff == True: #don't need a subplot for one of the y_vars
#         num_subplots = 1
#     else:
#         num_subplots = len(y_vars)
...
#     #for each of the y variables, it starts adding a new column for that event
#     for y in y_vars:
#         data, _ = datacacher.column_values_from_state(data, y, elapsed_seconds=True, time="scet")

#     data, _ = datacacher.column_values_from_state(data, x_var, elapsed_seconds=True, time="scet")
#     data, z_vals = datacacher.column_values_from_state(data, z_var, elapsed_seconds=True, time="scet") 
...
#     if diff == True and bar == True:
#         if len(y_vars) != 2:
#             raise Exception("Need 2 y_vars for difference plotting")
            
#         y1 = y_vars[0]
#         y2 = y_vars[1]
        
#         if plot_lines == True:
#             graph.add_trace(go.Scatter(x = data[x_var], y = data[y1]))
#             graph.add_trace(go.Scatter(x = data[x_var], y = data[y2]))

#         myData = data.copy()
#         myData = myData[[y1,y2,x_var]]

#         if absolute:
#             myData['diff'] = abs(myData[y1] - myData[y2])
#         else:
#             myData['diff'] = myData[y1] - myData[y2]
#         myDataNew = myData[[x_var, "diff"]].groupby(x_var).mean()
        
#         myDataNew['color'] = np.where(myDataNew["diff"] >= 0, 'rgb(0,255,0)', 'rgb(255,0,0)')
#         myDataNew['hovertext'] = "temp"
        
#         myDataNew["time"] = myDataNew.index
#         average_time = myDataNew[["diff", "time"]].diff(1)["time"].mean()

#         graph.add_traces(go.Bar(x=myDataNew.index, y = myDataNew["diff"], width=average_time.seconds*bar_width,
#                                 marker_color= myDataNew['color'], 
#                                 marker_line_color=myDataNew['color'],
#                                 opacity=0.6
#                                 ))
...
#         #helper function for difference coloring
#         def fillcol(label, higher_color, lower_color):
#             if label >= 1:
#                 return higher_color
#             else:
#                 return lower_color
...
#         myData['label'] = np.where(myData[y1] > myData[y2], 1, 0)
#         myData['group'] = myData['label'].ne(myData['label'].shift()).cumsum()
#         myData = myData.groupby('group')

#         dfs = []
#         for name, data in myData:
#             dfs.append(data)
        
#         #color for when y1 > y2
#         higher_color, unassigned_colors = palette.pop_next_color(unassigned_colors)
#         higher_color = higher_color.lstrip("#")
#         higher_color = 'rgba'+ str(tuple([int(higher_color[i:i+2], 16) for i in (0,2,4)] + [0.4]))
#         higher_color = 'rgba(0,255,0,0.4)'  # remove this line if you want color to come from pallete instead
    
        
#         #color for when y2 <= y1
#         lower_color, unassigned_colors = palette.pop_next_color(unassigned_colors)
#         lower_color = lower_color.lstrip("#")
#         lower_color = 'rgba'+ str(tuple([int(lower_color[i:i+2], 16) for i in (0,2,4)] + [0.4]))
#         lower_color = 'rgba(255,0,0,0.4)' # remove this line if you want color to come from pallete instead

#         for df in dfs:
#             graph.add_traces(go.Scatter(x= df[x_var], y = df[y1],
#                               line = dict(color='rgba(0,0,0,0)')))

#             graph.add_traces(go.Scatter(x=df[x_var], y = df[y2],
#                               line = dict(color='rgba(0,0,0,0)'),
#                               fill='tonexty', 
#                               fillcolor = fillcol(df['label'].iloc[0], higher_color, lower_color)))
...
#     for subplot_num, y_val in enumerate(y_vars, start=1):
#         print(y_val)
...
#             graph.add_vline(
#                 x= e[0].timestamp()*1000, 
#                 line_width=3, 
#                 line_color="black", 
#                 row = trace_num-1, 
#                 col= 1)

#             ## this way is most likely more reliable but has the annotation at the bottom
#             graph.add_annotation(
#                     xref='x',
#                     yref='paper',
#                     x= e[0].timestamp()*1000,
